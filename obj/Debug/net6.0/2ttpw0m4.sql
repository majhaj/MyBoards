...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Comments]') AND [c].[name] = N'Author');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Comments] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [Comments] DROP COLUMN [Author];
GO

ALTER TABLE [Comments] ADD [AuthorId] uniqueidentifier NOT NULL DEFAULT '00000000-0000-0000-0000-000000000000';
GO
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20221123123826_UserToCommentAdded', N'7.0.0');
GO
...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'State') AND [object_id] = OBJECT_ID(N'[workItemStates]'))
    SET IDENTITY_INSERT [workItemStates] ON;
INSERT INTO [workItemStates] ([Id], [State])
VALUES (1, N'To Do'),
(2, N'Doing'),
(3, N'Done');
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'Id', N'State') AND [object_id] = OBJECT_ID(N'[workItemStates]'))
    SET IDENTITY_INSERT [workItemStates] OFF;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20221123132947_WorkItemStateSeed', N'7.0.0');
GO
...
IF EXISTS (SELECT * FROM [sys].[identity_columns] WHERE [name] IN (N'State') AND [object_id] = OBJECT_ID(N'[workItemStates]'))
    SET IDENTITY_INSERT [workItemStates] ON;
...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20221123133329_AdditionWorkItemStateSeed', N'7.0.0');
GO
